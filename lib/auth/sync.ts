...
//       return newUser;
//     } else {
//       // User exists, check if we need to sync missing details or "upgrade" cookie consent from pending
//       const cookieStore = await cookies();
//       const consent = cookieStore.get('cookie_consent')?.value || 'pending';
...
//     return dbUser;
//   } catch (error) {
//     // Gracefully handle cases where Clerk user is deleted but session persists
//     console.error("Clerk sync error (possible deleted user):", error);
//     return null;
//   }
// }
