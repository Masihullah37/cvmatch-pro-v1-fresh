...
export default async function DashboardPage({
  ...
  // 🌟 CRASH PREVENTION: Ensure the newly created record is assigned correctly and awaited
  if (!dbUser && user) {
    try {
      ...
      console.error("Failed to sync user to database on login:", dbError);
    }
  }

  // 🌟 If database sync is still catching up on a slow mobile link, render a clean loading fallback
  // instead of crashing the layout engine into an APP_RUNTIME_ERROR
  if (!dbUser) {
    return (
  ...
  // 🌟 Added optional chaining safeguards to prevent property crashes on dynamic mounts
  const credits = dbUser?.credits ?? 0;
  const bestScore = analyses.length > 0 ? Math.max(...analyses.map(a => a.atsScore || 0)) : 0;
  const planLabel = dbUser?.plan === 'monthly' ? 'Pro' : dbUser?.plan === 'one_time' ? 'Starter' : 'Gratuit';
