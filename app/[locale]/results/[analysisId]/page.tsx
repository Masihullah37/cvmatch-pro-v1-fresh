...
export default async function ResultsPage({
  ...
  // Explicit breakdown type mapping
  const breakdown = (analysis.scoreBreakdown as Record<string, any>) || {};

  // 2. Compute total using the parsing utility mapping both French and English schemas
  let calculatedTotal = hasATSAnalysis
    ? (parseSafeNumber(breakdown.keywords || breakdown.mots_cles) +
      parseSafeNumber(breakdown.format || breakdown.format_structure) +
      parseSafeNumber(breakdown.experience) +
      parseSafeNumber(breakdown.training || breakdown.formation) +
      parseSafeNumber(breakdown.skills || breakdown.competences) +
      parseSafeNumber(breakdown.readability || breakdown.lisibilite))
    : 0;

  // 3. ULTIMATE SAFETY BACKUP: Fall back immediately to db aggregate score if calculations are empty or NaN
  if (!calculatedTotal || isNaN(calculatedTotal)) {
    calculatedTotal = Math.round(Number(analysis.atsScore ?? 0));
  }

  // Final sanitized configuration score allocation
  const score = Math.max(0, Math.min(100, calculatedTotal));
