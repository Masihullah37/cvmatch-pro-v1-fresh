...
import { useEffect } from 'react';
import { AlertCircle, Home, RotateCcw, RefreshCw } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { useClerk } from '@clerk/nextjs';
...
export default function Error({
    ...
    // Detect if this is actually a Clerk auth error or session crash
    ...
    const isAuthError =
        ...
        errorDigest.includes("auth") ||
        errorDigest === "auth_sync_failed";

    useEffect(() => {
        // Log the error for debugging purposes
        console.error('SYSTEM_CRASH_LOGGED:', error);
    }, [error]);
    ...
    const handleGenericRetry = () => {
        // Simply try to recover the layout without logging the user out!
        reset();
    };
    ...
    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 font-sans">
            <div className="max-w-md w-full bg-white rounded-[3rem] shadow-[0_32px_64px_-12px_rgba(0,0,0,0.1)] border border-slate-100 p-8 md:p-12 text-center animate-in fade-in zoom-in-95 duration-500">
                ...
                {isAuthError ? (
                    /* CASE A: Genuine Session / Auth Error */
                    <>
                        <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight leading-tight">
                            Oups ! Session <span className="text-red-500">Expirée</span>
                        </h2>
                    ...
                    /* CASE B: Generic Layout / Runtime Error (like Download interruptions) */
                    <>
                        <h2 className="text-3xl font-black text-slate-900 mb-4 tracking-tight leading-tight">
                        ...
                        <p className="text-slate-500 text-lg font-medium leading-relaxed mb-10">
                            Nous avons rencontré une perturbation temporaire. Cela peut être dû à une micro-coupure ou un conflit d'affichage. Veuillez réessayer de charger la page.
                        </p>

                        <div className="flex flex-col gap-4 w-full">
                            <button
                                onClick={handleGenericRetry}
                                className="w-full bg-slate-900 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl shadow-slate-200 hover:bg-slate-800 transition-all active:scale-95 flex items-center justify-center gap-3"
                            >
