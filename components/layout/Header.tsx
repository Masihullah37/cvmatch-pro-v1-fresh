...
export default function Header() {
  const { userId } = useAuth();
  const router = useRouter();
  const pathname = usePathname() || ''; // Safely fallback to empty string if null
  const locale = useLocale();

  const [scrolled, setScrolled] = useState(false);
  const [mobileOpen, setMobileOpen] = useState(false);
  const [isAdmin, setIsAdmin] = useState(false);
  const [isHome, setIsHome] = useState(true); // Default to true to prevent button flashes on first mount

  useEffect(() => {
    // 🌟 Safe evaluation on the client side after mounting
    const checkingHome =
      pathname === '/' ||
      pathname === '/fr' ||
      pathname === '/en' ||
